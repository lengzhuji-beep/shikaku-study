/**
 * 資格対策ドットコム - FP1級過去問 過去5年分（全15回・750問）動的レンダリングスクリプト
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!container || !window.FP1_PAST_DATA) return;
  ...
  let currentSession = '2024-09'; // デフォルトは最新2024年9月
  let currentCat = 'all';
  let currentMode = 'all';
  ...
  modeSwitchBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (currentMode === 'bookmark') {
        ...
        currentSession = activeBtn ? activeBtn.getAttribute('data-session') : '2024-09';
        showSelectionUI();
      }
    });
  });
  ...
  function renderQuestions() {
    ...
    if (currentSession === 'all') {
      for (const sid of Object.keys(window.FP1_PAST_DATA)) {
        const qs = window.FP1_PAST_DATA[sid].map(q => ({...q, sessionId: sid}));
        questions.push(...qs);
      }
    } else {
      questions = window.FP1_PAST_DATA[currentSession] || [];
    }
    ...
    // ブックマークフィルター
    if (currentMode === 'bookmark') {
      const bmQuestions = questions.filter(q => bookmarks.includes(`fp1-past-${q.sessionId || currentSession}-${q.num}`));
    ...
    function renderSinglePastQuestion() {
      ...
      const qId = `fp1-past-${item.sessionId || currentSession}-${item.num}`;
      ...
      card.innerHTML = `
        ...
        <div class="quiz-explanation-area" style="display:none;">
          ...
          <div class="next-question-bar" style="margin-top:20px; text-align:right;">
            <button type="button" class="next-question-btn" id="nextFp1QuestionBtn" style="padding:12px 28px; font-size:1.05rem; font-weight:bold; background:#345d4d; color:white; border:none; border-radius:9999px; cursor:pointer; box-shadow:0 4px 12px rgba(52,93,77,0.22); transition:0.2s;">
      ...
      // ブックマーク処理
      const bmBtn = card.querySelector('#currentFp1BmBtn');
      ...
      options.forEach(opt => {
        opt.addEventListener('click', () => {
          ...
          const nextBtn = expArea.querySelector('#nextFp1QuestionBtn');
    ...
    function showFp1ResultCard() {
      ...
      resCard.innerHTML = `
        ...
        <h2 style="font-size:1.8rem; margin-bottom:10px; color:#2d3748;">
          FP1級 ${sessionTitle} 演習完了！
        </h2>
        <p style="font-size:1.1rem; color:#718096; margin-bottom:25px;">
          ${isPassed ? 'お見事です！合格ライン（60%以上）を見事に達成しました！難関FP1級への大きな前進です！' : '惜しい！合格ラインは60%以上です。高度な論点を解説でしっかり復習して再挑戦しましょう。'}
        </p>
      ...
      resCard.querySelector('#retryFp1ExamBtn').addEventListener('click', () => {
        renderQuestions();
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });

      resCard.querySelector('#chooseOtherFp1Btn').addEventListener('click', () => {
        showSelectionUI();
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
    }
        ...
        options.forEach(opt => {
          opt.addEventListener('click', () => {
            ...
            if (isCorrect) {
              expArea.classList.add('is-correct');
              resTitle.className = 'quiz-result-title result-correct';
              resTitle.innerHTML = '<i class="fas fa-check-circle"></i> 正解です！';
            } else {
              expArea.classList.add('is-wrong');
              resTitle.className = 'quiz-result-title result-incorrect';
              resTitle.innerHTML = `<i class="fas fa-times-circle" style="color:#e53e3e;"></i> 不正解…（正解は ${item.correct}）`;
  ...
  if (printPastExamBtn) {
    printPastExamBtn.addEventListener('click', () => {
      ...
      const questions = window.FP1_PAST_DATA[currentSession] || [];
      ...
      openPastExamPrintWindow(questions, `FP1級 学科基礎編 ${sessionName}`);
    });
  }
  ...
  function openPastExamPrintWindow(questions, customTitle) {
    ...
    const examTitle = customTitle || 'FP1級 過去問演習';
...
<body>
  ...
  <div class="print-paper">
    ...
    <div class="print-header">
      ...
      <div class="print-subtitle">資格対策ドットコム ｜ 制限時間: 150分 ｜ 合格基準: 50問中30問以上正解</div>
    </div>
