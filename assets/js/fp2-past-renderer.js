...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!container || !window.FP2_PAST_DATA) return;
  ...
  function renderQuestions() {
    ...
    if (currentSession === 'all') {
      for (const sid of Object.keys(window.FP2_PAST_DATA)) {
        const qs = window.FP2_PAST_DATA[sid].map(q => ({...q, sessionId: sid}));
        questions.push(...qs);
      }
    } else {
      questions = window.FP2_PAST_DATA[currentSession] || [];
    }
    ...
    // ブックマークフィルター
    if (currentMode === 'bookmark') {
      const bmQuestions = questions.filter(q => bookmarks.includes(`fp2-past-${q.sessionId || currentSession}-${q.num}`));
      ...
      if (total === 0) {
        container.innerHTML = `
          <div class="card" style="text-align:center; padding:35px;">
            <p style="color:#718096;">該当する問題がありませんでした。他の実施回または分野を選択してください。</p>
          </div>
        `;
        return;
      }
    ...
    function renderSinglePastQuestion() {
      ...
      const qId = `fp2-past-${item.sessionId || currentSession}-${item.num}`;
      ...
      card.innerHTML = `
        <div class="quiz-header-row">
          ...
          <button type="button" class="bookmark-toggle-btn ${isBookmarked ? 'is-bookmarked' : ''}" id="currentFp2BmBtn">
        ...
        <div class="quiz-explanation-area" style="display:none;">
          ...
          <div class="next-question-bar" style="margin-top:20px; text-align:right;">
            <button type="button" class="next-question-btn" id="nextFp2QuestionBtn" style="padding:12px 28px; font-size:1.05rem; font-weight:bold; background:#345d4d; color:white; border:none; border-radius:9999px; cursor:pointer; box-shadow:0 4px 12px rgba(52,93,77,0.22); transition:0.2s;">
      ...
      // ブックマーク処理
      const bmBtn = card.querySelector('#currentFp2BmBtn');
      ...
      options.forEach(opt => {
        opt.addEventListener('click', () => {
          ...
          if (isCorrect) {
            ...
            resTitle.className = 'quiz-result-title result-correct';
            resTitle.innerHTML = '<i class="fas fa-check-circle" style="color:#38a169;"></i> 正解です！';
          } else {
            expArea.classList.add('is-wrong');
            resTitle.className = 'quiz-result-title result-incorrect';
            resTitle.innerHTML = `<i class="fas fa-times-circle" style="color:#e53e3e;"></i> 不正解…（正解は ${item.correct}）`;
          }

          const nextBtn = expArea.querySelector('#nextFp2QuestionBtn');
    ...
    function showFp2ResultCard() {
      ...
      resCard.innerHTML = `
        ...
        <div style="display:flex; justify-content:center; gap:15px; flex-wrap:wrap;">
          <button type="button" id="retryFp2ExamBtn" class="btn" style="background:#3182ce; color:white; padding:12px 24px; font-weight:bold; border-radius:6px; border:none; cursor:pointer;">
            <i class="fas fa-redo"></i> もう一度この回を解く
          </button>
          <button type="button" id="chooseOtherFp2Btn" class="btn" style="background:#edf2f7; color:#4a5568; padding:12px 24px; font-weight:bold; border-radius:6px; border:1px solid #cbd5e0; cursor:pointer;">
      ...
      resCard.querySelector('#retryFp2ExamBtn').addEventListener('click', () => {
        renderQuestions();
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });

      resCard.querySelector('#chooseOtherFp2Btn').addEventListener('click', () => {
        showSelectionUI();
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
    }
  ...
  function updateProgress(current, total) {
    ...
    if (text) {
      if (total === 0) {
        text.innerText = '進捗: 0問';
      } else {
        const pct = Math.round((current / total) * 100);
        text.innerHTML = `進捗: <strong>${current}</strong> / ${total} 問完了 (${pct}%) ｜ 正解数: <strong>${correctCount}</strong> 問`;
      }
    }
    if (fill) {
  ...
  if (printPastExamBtn) {
    printPastExamBtn.addEventListener('click', () => {
      ...
      const questions = window.FP2_PAST_DATA[currentSession] || [];
      ...
      openPastExamPrintWindow(questions, `FP2級 学科試験 ${sessionName}`);
    });
  }
  ...
  function openPastExamPrintWindow(questions, customTitle) {
    ...
    const examTitle = customTitle || 'FP2級 過去問演習';
